...
def get_filtered_data(df, selected_companies):
    df_filtered = df[df['company_name'] == selected_companies].reset_index(drop=True)
    return df_filtered
...
def build_lollipop_chart(accomplishment, color_accomplishment, companies_names):
...
def chart_left(selected_companies):
    ...
    return accomplishment, color_accomplishment, companies_names
...
def generate_chart_left(selected_companies):
    accomplishment, color_accomplishment, companies_names = chart_left(selected_companies)
    
    return html.Div(children=[
        html.Div("Concernant leurs propres émissions", className="h5 p-3"),
        html.Div(
        dcc.Graph(figure=build_bullet_gauge(accomplishment, color_accomplishment, companies_names),
                    style={
                        'width': '40%',
                        'min-width': '40%',
                        'max-width': '40%',
...
def generate_chart_right(selected_companies):
    accomplishment, color_accomplishment, companies_names = chart_right(selected_companies)
